...
Любой новый агент/разработчик начинает работу < 2 минут:
START_HERE.md → docs/* → verify_environment.py → run_stream.py.

## Next Goal

Не зафиксирована владельцем проекта. Кандидаты (из отложенного):
LLM-подключение (llm/ пуст, следующий пункт ROADMAP), либо улучшение
конвейера STT (overlap, partial results) — решение за владельцем.
